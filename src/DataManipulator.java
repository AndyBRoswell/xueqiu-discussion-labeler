...
//	private static final TreeSet<Integer> FinalSearchResult = new TreeSet<>();
	...
	public static boolean AddLabel(int Index, String Category, String Label) {
		...
		final LabelStatus TargetLabelStatus = TargetCat.get(Label);
		if (TargetLabelStatus.LabeledAtThisTime == false) {
			final Integer Count = TargetLabelStatus.LabeledCount; // 增加 1 次被标注次数
			if (Count == null) TargetCat.put(Label, new LabelStatus(1));
			else TargetCat.put(Label, new LabelStatus(Count + 1));
		}
		return TargetLabelStatus.LabeledAtThisTime;
	}
...
//	}
	...
	public static boolean DeleteLabel(int Index, String Category, String Label) {
		...
		HashMap<String, LabelStatus> TargetCat = TargetLabels.get(Category); // 获得该标签所属的类
		final LabelStatus TargetLabelStatus = TargetCat.get(Label);
		if (TargetLabelStatus.LabeledAtThisTime == true) {
			final Integer Count = TargetLabelStatus.LabeledCount;
