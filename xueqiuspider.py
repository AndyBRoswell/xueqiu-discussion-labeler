...
def save_file(content_list):
    with open('xueqiu.json', 'w')as f:
        f.write("{ \n""name:[ \n")
    with open('xueqiu.json', 'a')as f:
...
def parse_comment_url( url):
    r = requests.get(url, headers=headers, proxies=proxies, verify=False)  # 发送请求
...
for i in range(3):
    ...
    try:
        ...
        comment_list, count = parse_comment_url(detail_url)
        time.sleep(5)  # 等待5秒，时间越长越不容易被网站检测到
    except Exception as e:  # 捕获错误信息
        print("Error:", e)
        time.sleep(5)
        comment_list = parse_comment_url(detail_url)  # 失败后再次尝试获取
        time.sleep(5)
    comment_data.append(comment_list)
